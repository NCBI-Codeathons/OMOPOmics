...
dirs$code <- file.path(dirs$base,"src")
base_dir<- file.path(dirs$code,"R/atac_example")
filter  <- dplyr::filter
select  <- dplyr::select
mutate  <- dplyr::mutate
...
```{r loadDataSQL,eval=FALSE}
 file.path(dirs$data,"cohorts/cohort1.txt") %>%
...
```{r loadDataManual}
file_table  <- file.path(base_dir,"GSE60682_details.tsv") %>%
                read.table(sep="\t",header = TRUE,stringsAsFactors = FALSE) %>%
                as_tibble() %>%
