...
dirs      <- list(base=file.path(here::here()))
filter    <- dplyr::filter
select    <- dplyr::select
mutate    <- dplyr::mutate
...
Database includes samples with activation of `Human` `CD4+ T-cells` in `normal donors` at `0,2,4,` and `6 hours post activation`. OMOPOmics allows for basic SQL queries which retreieve these sample file locations along with columns of additional treatment information.
...
tst_table   <- read.table(input_file,sep=",",header = TRUE,stringsAsFactors = FALSE) %>%
                as_tibble() %>%
                mutate(Treatment=perturbation_dose_value_as_number)
...
## 4. Read in associated data files.
...
names(beds)       <- select(tst_table,file_source_value) %>% unlist() %>% basename()
```
