...
dirs      <- list()
dirs$base <- file.path('/data/projects/andrew/ROMOPOmics_imp')
...
#dm_lst<- loadDataModel(master_table_file = file.path(dirs$base,"OMOP_CDM_v6_0_custom.csv"),as_table_list = TRUE) #Load custom model
dm_lst<- loadDataModel(as_table_list = TRUE)
```

```{r loadExampleModels,echo=FALSE}
dm      <- loadDataModel(master_table_file = file.path(dirs$base,"OMOP_CDM_v6_0_custom.csv"))
dm_old  <- loadDataModel() #Load old model first.
```
...
omop_tabs       <- lapply(seq_input_files,readInputFiles,data_model = dm,mask_table=msks$sequencing_mask) %>%
                    combineInputTables()
```
